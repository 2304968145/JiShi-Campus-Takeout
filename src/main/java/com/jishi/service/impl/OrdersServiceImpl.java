...
public class OrdersServiceImpl extends ServiceImpl<OrdersMapper, Orders>
    ...
    public R submit(Orders orders) {
        ...
        if(shoppingCarts == null || shoppingCarts.size() == 0){
            throw new BussinessException("购物车为空，不能下单");
        }
        ...
        if(addressBook == null){
            throw new BussinessException("用户地址信息有误，不能下单");
        }

        long orderId = IdWorker.getId();//订单号

        //原子Integer类型，防止高并发数值计算出错
        AtomicInteger amount = new AtomicInteger(0);

        //一条订单对应多个订单细节（即不同的菜品和套餐）
        //这里是后端计算总金额，并把购物车对象转换成订单细节对象（两个其实差不多，可以直接拷贝）
        ...
        //相当于业务ID，公司里面经常不以生成的主键id作为业务ID，会按一定规则生成唯一业务ID
